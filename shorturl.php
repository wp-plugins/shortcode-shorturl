...
function jhm_shorturl_set($post) {
	...
	switch($_s["provider"]) {
			   ...
			   case "unu": 	$_r = 'http://u.nu/unu-api-simple?url=' . urlencode($post->guid); break;
			   case "isgd": $_r = 'http://is.gd/api.php?longurl=' . $post->guid; break;
			   case "snurl": $_r = 'http://sn.im/site/snip?r=simple&link=' . urlencode($post->guid); break;
			   case "tiny": $_r = 'http://tinyurl.com/api-create.php?url=' . urlencode($post->guid); break;
			   case "trim": $_r = 'http://api.tr.im/api/trim_simple?url=' . urlencode($post->guid); break;
			}
			
   # Else use bit.ly the default provider
   if(!$_r) { $_r = 'http://bit.ly/api?url=' . urlencode($post->guid); }
...
function jhm_shorturl_settings() {
	...
	if($_POST["store_shorturl_settings"]) {
		$_s["provider"] = empty2zero($_POST["provider"]);
		$_s["auto"] = $_POST["auto"];
